...
function doGet(e) {
  ...
  try {
    if (action === "linkedinCallback") return linkedinCallback(e);
    if (action === "instagramCallback") return instagramCallback(e);
    if (action === "threadsCallback") return threadsCallback(e);
    if (action === "metaCallback") return metaCallback(e);
    ...
    return jsonResponse(invalidImportActionResponse_(action));
  } catch (err) {
...
function doPost(e) {
  ...
  try {
    ...
    return jsonResponse({
      ...
      routeStatus: action === "savePost" ? getDeploymentDiagnostics().writePostRouteStatus : undefined
    });
  }
}
...
function getPostsSheet_() {
  return getRequiredSheetByName_(SHEETS.POSTS);
}
...
function migrateSheetOrderAndHeaders() {
  ...
  return {
    ...
    reason: "Safe automatic sheet reordering/header migration is not applied by Code.gs. Reorder tabs manually and keep existing formulas intact.",
...
function getWorkspaceServerConfig_(payload) {
  ...
  return {
    WORKSPACE_ID: String(pickFirstDefined_(payload.workspace_id, payload.workspaceId, props.getProperty("WORKSPACE_ID"), WORKSPACE_SERVER_DEFAULTS.WORKSPACE_ID)).trim() || WORKSPACE_SERVER_DEFAULTS.WORKSPACE_ID,
    WORKSPACE_SLUG: String(pickFirstDefined_(payload.workspace_slug, payload.workspaceSlug, props.getProperty("WORKSPACE_SLUG"), WORKSPACE_SERVER_DEFAULTS.WORKSPACE_SLUG)).trim() || WORKSPACE_SERVER_DEFAULTS.WORKSPACE_SLUG,
    SHEET_ID: String(pickFirstDefined_(payload.sheet_id, payload.sheetId, props.getProperty("SHEET_ID"), props.getProperty("SPREADSHEET_ID"), WORKSPACE_SERVER_DEFAULTS.SHEET_ID)).trim(),
...
function savePost(payload) {
  ...
  const settings = getSettingsRegistry();
  const defaultPillar = settings.pillars[0] || "authority";
  const existingRow = findPostObjectById_(pickFirstDefined_(payload.postId, payload.id));
  ...
  const finalExistingRow = findPostObjectById_(pickFirstDefined_(payload.postId, payload.id));
  ...
  upsertPostObjectById_(sheet, normalized);
...
function getDiagnostics(payload) {
  if (payload && payload.activeDeploymentUrlFromFrontend) {
    setScriptProp_("ACTIVE_DEPLOYMENT_URL_FROM_FRONTEND", String(payload.activeDeploymentUrlFromFrontend).trim());
  }
...
function saveAIProviderConfig(payload) {
  ...
  if (apiKey) {
    setScriptProp_(config.apiKeyKey, apiKey);
  }
  if (apiUrl) {
    setScriptProp_(providerId.toUpperCase() + "_API_URL", apiUrl);
  }
  if (!apiKey && !apiUrl) {
  ...
  return {
    ...
    hasApiUrl: !!apiUrl || !!config.defaultApiUrl
  };
}
